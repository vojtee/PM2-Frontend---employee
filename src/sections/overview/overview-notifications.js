...
export const OverviewNotifications = (props) => {
    ...
    const rowsPerPage = 4; // Set the default number of notifications per page

    const handleChangePage = (event, newPage) => {
        setPage(newPage);
    };

    // Paginate the notifications based on the current page and rows per page
                    ...
                    {paginatedNotifications.map((notif, index) => {
                        ...
                        return (
                            <ListItem divider={hasDivider} key={notif.id}>
                                {/* You can customize the Avatar or use the ListBulletIcon */}
                                <ListItemText
